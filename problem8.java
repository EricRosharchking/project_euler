    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < list.size(); i ++) {
            if (list.get(i).length() >= 13) {
                for (int j = 0; j <= list.get(i).length() - 13; j ++) {
                    stringList.add(list.get(i).substring(j, j + 13));
                }
            }
        }
        ...
        for (String string: stringList) {
            long intermediate = 1l;
            List<String> intStringList = Arrays.asList(string.split(""));
            for (String intString: intStringList) {
                intermediate *= Long.parseLong(intString);
            }
